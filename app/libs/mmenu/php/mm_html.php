...
class MmHtml {
	...
	protected function html_input( $optn, $type = 'text', $extr = '' )
	{
		$v = $this->html_input_get_vars( $optn );
		return '
			<input type="' . $type . '" name="' . $v[ 'name' ] . '" id="' . $v[ 'id' ] . '" value="' . esc_attr( stripslashes( $v[ 'value' ] ) ) . '"' . ( $extr ? ' ' . $extr : '' ) . ' />';
	}
	protected function html_textarea( $optn )
	{
		$v = $this->html_input_get_vars( $optn );
		return '
			<textarea name="' . $v[ 'name' ] . '" id="' . $v[ 'id' ] . '">' . esc_attr( stripslashes( $v[ 'value' ] ) ) . '</textarea>';
	}
	...
	protected function html_input_get_vars( $optn )
	{
		return array(
			'id'	=> $optn[ 1 ] . '_' . $optn[ 2 ],
			'name' 	=> $optn[ 1 ] . '[' . $optn[ 2 ] . ']',
